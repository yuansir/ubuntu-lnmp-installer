...
MySQL_Opt()
{
    if [[ ${memory_total} -gt 1024 && ${memory_total} -lt 2048 ]]; then
    ...
    elif [[ ${memory_total} -ge 2048 && ${memory_total} -lt 4096 ]]; then
    ...
    elif [[ ${memory_total} -ge 4096 && ${memory_total} -lt 8192 ]]; then
    ...
    elif [[ ${memory_total} -ge 8192 && ${memory_total} -lt 16384 ]]; then
    ...
    elif [[ ${memory_total} -ge 16384 && ${memory_total} -lt 32768 ]]; then
    ...
    elif [[ ${memory_total} -ge 32768 ]]; then
...
Do_Query()
{
    ...
    ${mysql_install_dir}/bin/mysql --defaults-file=~/.my.cnf </tmp/.mysql.tmp
    return $?
}
...
Install_MySQL() {
    ...
    [ ! -d ${mysql_install_dir} ] && make -p ${mysql_install_dir}
    [ ! -d ${mysql_data_dir} ] && make -p ${mysql_data_dir}
...
EOF
    ...
    ${mysql_install_dir}/scripts/mysql_install_db --defaults-file=/etc/my.cnf --basedir=${mysql_install_dir} --datadir=${mysql_data_dir} --user=mysql
...
EOF
    ldconfig
    ln -sf ${mysql_install_dir}/lib/mysql /usr/lib/mysql
    ...
    ln -sf ${mysql_install_dir}/bin/mysqld_safe /usr/bin/mysqld_safe
    ln -sf ${mysql_install_dir}/bin/mysqlcheck /usr/bin/mysqlcheck

    ${mysql_install_dir}/bin/mysqladmin -u root password "${dbrootpwd}"
    if [ $? -ne 0 ]; then
...
EOF
        ...
        ${mysql_install_dir}/bin/mysql --defaults-file=~/.emptymy.cnf -e "UPDATE mysql.user SET Password=PASSWORD('${dbrootpwd}') WHERE User='root';"
    ...
    Do_Query "UPDATE mysql.user SET Password=PASSWORD('${dbrootpwd}') WHERE User='root';"
    ...
    rm -rf  ${base_dir}/src/mysql-5.6.29
    cd ${base_dir}
}
